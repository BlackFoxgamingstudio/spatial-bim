# Developer Guide: Sovereign Spatial BIM
...
```bash
# 1. Clone
git clone https://github.com/BlackFoxgamingstudio/spatial-bim.git
cd spatial-bim
...
```bash
export SBB_IFC_SCHEMA_VERSION=your_value_here
export SBB_CLASH_TOLERANCE_MM=your_value_here
export SBB_MESH_LOD=your_value_here
export SBB_AR_TARGET_PLATFORM=your_value_here
...
│   ├── cli.py               # Typer CLI (sovereign-spatial-bim)
...
```bash
# Build
docker build -t sovereign-spatial-bim:dev .

# Run with env file
docker run --env-file .env -p 8780:8780 sovereign-spatial-bim:dev

# Full stack (with n8n network)
docker-compose up --build

# Logs
docker-compose logs -f sovereign-spatial-bim
...
3. Use **SovereignTools** node → configure endpoint to `http://localhost:8780`
...
| Issue | Diagnosis | Fix |
|-------|-----------|-----|
| Port `8780` in use | `lsof -i :8780` | Kill PID or change `SERVICE_PORT` |
