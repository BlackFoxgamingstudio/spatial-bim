# Engineering Roadmap & Implementation Status — Sovereign Spatial BIM

**Package ID**: `PKG-000`  
**Domain**: ** 3D Graphics & Spatial BIM  
**Microservice Port**: `http://127.0.0.1:8780`  
...
| **Networking & API** | REST Microservice (`PORT 8780`) | ✅ Complete | OpenAPI 3.1 spec, Swagger UI at `/docs` |
...
### Component 1: `IFCParser`
- **Role**: Handles all ifcparser operations. Exposes async methods callable from the core dispatcher.
- **Current Status**: ⏳ Queued for v1.1.0 Implementation
- **Integration**: Exposed via `POST /api/v1/execute` with action `ifcparser`
- **Verification Strategy**: Dedicated `unittest.TestCase` validating deterministic output and idempotency.

### Component 2: `ClashDetector`
- **Role**: Handles all clashdetector operations. Exposes async methods callable from the core dispatcher.
- **Current Status**: ⏳ Queued for v1.1.0 Implementation
- **Integration**: Exposed via `POST /api/v1/execute` with action `clashdetector`
- **Verification Strategy**: Dedicated `unittest.TestCase` validating deterministic output and idempotency.

### Component 3: `MeshOptimizer`
- **Role**: Handles all meshoptimizer operations. Exposes async methods callable from the core dispatcher.
- **Current Status**: ⏳ Queued for v1.1.0 Implementation
- **Integration**: Exposed via `POST /api/v1/execute` with action `meshoptimizer`
- **Verification Strategy**: Dedicated `unittest.TestCase` validating deterministic output and idempotency.

### Component 4: `SpatialQueryEngine`
- **Role**: Handles all spatialquery operations. Exposes async methods callable from the core dispatcher.
- **Current Status**: ⏳ Queued for v1.1.0 Implementation
- **Integration**: Exposed via `POST /api/v1/execute` with action `spatialqueryengine`
- **Verification Strategy**: Dedicated `unittest.TestCase` validating deterministic output and idempotency.

### Component 5: `ARExporter`
- **Role**: Handles all arexporter operations. Exposes async methods callable from the core dispatcher.
- **Current Status**: ⏳ Queued for v1.1.0 Implementation
- **Integration**: Exposed via `POST /api/v1/execute` with action `arexporter`
- **Verification Strategy**: Dedicated `unittest.TestCase` validating deterministic output and idempotency.
...
1. Create discrete module file: `src/spatial_bim_<component>.py`
